...
В данной документации представлены **компоненты высшего порядка** (HOC - higher-order component), иногда наываемые enhancers (усилители)
HOC - это функция, которая принимает React компонент и возвращает новый React компонент.

```js
const EnhancedComponent = hoc(BaseComponent)
```

Функция compose позволяет компоновать несколько HOC компонентов:
...
Большинство Recompose хелперов это **функции которые возвращают компоненты высшего порядка (HOC)**:
...
Некоторые хелперы, такие, как `pure`, сами являются компоненентами высшего порядка:
