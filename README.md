...
Recompose это React утилита для создания функциональных компонентов и компонентов высшего порядка.

[**Полная API документация**](docs/API.md) - Узнайте о каждом хелпере

[**Recompose Base Fiddle**](https://jsfiddle.net/evenchange4/p3vsmrvo/1599/) - Поэксперементируйте с Recompose
...
[**recompose-relay**](src/packages/recompose-relay) — Recompose хелперы для Relay

## Вы можете использовать Recompose для...

### ...определить состояние в stateless компонентах

Такие хелперы как `withState()` и `withReducer()` предоставляют более удобный способ для выражения обновления состояния:
...
Или используя a Redux-style редьюсер:
...
### ...выполнять наиболее распространенные React паттерны

Такие хелперы как `componentFromProp()` и `withContext()` инкапсулируют общие React паттерны в общий функциональный интерфейс:
